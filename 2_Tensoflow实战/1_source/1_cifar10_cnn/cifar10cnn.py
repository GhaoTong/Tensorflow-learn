...
images_test , lables_test = cifar10_input.inputs (eval_data = True ,
                                                data_dir = data_dir,
                                                batch_size = batch_size)

image_holder = tf.placeholder(tf.float32 , [batch_size , 24,24,3] )
labele_holder = tf.placeholder(tf.int32, [batch_size])
...
for step in range(max_steps):
    ...
    _,loss_value = sess.run([train_op,loss],
        feed_dict={image_holder : image_batch, labele_holder:label_batch })
...
while step < num_iter:
    ...
    predictions = sess.run([top_k_op], feed_dict={image_holder: image_batch,labele_holder: label_batch})
    ture_count += np.sum(predictions)
    step += 1
precision = ture_count/total_sample_count 
print('precsion @ 1 =%.3f' % precision )

non_use = tf.placeholder(tf.float32 , [batch_size , 24,24,3] )
